...
async function processPDI() {
  try {
    ...
    for (const row of qrRes.recordset) {
      ...
      // 4️⃣ UPDATE STATUS
      const PDI_VALUE =
        "OK:NA:1,20,21,22,3009,3010,3011,3012,3013,3014,7,8,9,10,11,12,13,14,3015,3016,4,15,16,18,19,3,30,3021,3022,3023,3024,3026,28,33,39,32,2,29,5,35,40,41,3025";
      ...
      const updateRes = await poolPackStatus
        .request()
        .input("pdi", sql.NVarChar(sql.MAX), PDI_VALUE)
        .input("pid", sql.Int, packID)
        .input("pno", sql.Int, packNo)
        .query(updateSql);
